...
class Organization(Base):
    ...
    # Cifrato a riposo (Fase 8): P.IVA esplicitamente richiesta dalla roadmap tra i dati
    # sensibili da proteggere. Colonna TEXT (non String(32)): il testo cifrato è più
    # lungo del valore in chiaro. Vedi app/db/encrypted_types.py.
    vat_number: Mapped[str | None] = mapped_column(EncryptedString(), nullable=True)
    sector: Mapped[str | None] = mapped_column(String(120), nullable=True)
    employee_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    annual_revenue_eur: Mapped[float | None] = mapped_column(
